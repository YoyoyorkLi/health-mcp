...
app.get("/authorize", async (c) => {
	...
	return renderApprovalDialog(c.req.raw, {
		...
		csrfToken,
		server: {
			description: "Read-only access to your Pulse sleep / HRV / recovery / drinking data, gated to one GitHub account.",
...
app.get("/callback", async (c) => {
	...
	// This connector exists to expose one person's health data. Refuse to
	// issue a token to anyone else's GitHub account, no matter how they
	// reached this callback -- this is the actual access-control boundary,
	// not the approval dialog above (which any GitHub user can click through).
	if (login !== c.env.ALLOWED_GITHUB_LOGIN) {
		return c.text("Forbidden: this connector is restricted to a single GitHub account.", 403);
	}
